...
class TeamquestSpider(scrapy.Spider):
    ...
    def _extract_languages(self, technology_set):
        try:
            return set(item for item in technology_set if item.lower() in TeamquestSpider.languages)
        except IndexError:
            return None

    def _get_languages_and_technologies_set(self, ancestor):
        try:
            return set(technology for technology in ancestor.css('.tag-xs::text').extract())
        except IndexError:
            return None
    ...
    def _get_salary_forks(self, ancestor):
        ...
        forks = [actual_forks for actual_forks in forks if actual_forks.strip() != '']
